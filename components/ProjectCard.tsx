...
export function ProjectCard({
  ...
  return (
    <div className={`group rounded-xl overflow-hidden transition-all duration-300 border border-gray-700 ${featured ? 'shadow-xl hover:shadow-2xl shadow-blue-500/20 hover:shadow-blue-500/30' : 'shadow-lg hover:shadow-xl'}`}>
      ...
      <div className="bg-gray-900 p-6 border-t border-gray-800 flex flex-col h-full">
        <div className="flex items-start justify-between mb-2">
          ...
          {projectType && (
            <span className={`text-xs px-2 py-1 rounded-md font-medium ${
              projectType === 'Client' ? 'bg-green-900 text-green-200' :
              projectType === 'Venture' ? 'bg-purple-900 text-purple-200' :
              projectType === 'Employment' ? 'bg-blue-900 text-blue-200' :
        ...
        <div className="mt-auto">
          <Link
            href={link}
            className="inline-flex items-center justify-center w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-3 px-4 rounded-md transition-colors"
          >
